...
running 22 semantic deep-review checks. The deep-review checklist lives in
...
After Phase 3, Pierre runs the **Review Simulator**: 22 read-only, evidence-based checks the
static scanner cannot fully judge. The full checklist, per-check procedure, and output format live
in [`references/pierre-deep-review.md`](references/pierre-deep-review.md) — read it before starting
Phase 4.

**What Pierre does:** read Swift, metadata, xcstrings, paywall views, screenshot images; fetch
privacy and support URLs; cross-check claims vs code, policy vs SDK usage, screenshots vs features,
and paywall disclosure quality.

**Rules (summary):**

- Run **all 22 checks every time** — report each as `REVIEW-PASS:` or `REVIEW-FINDING:` (never skip).
...
| 4 | **2.3.2** | Primary category fits app type? |
| 5 | **2.3.5** | Screenshots match shipped features? |
| 6 | **2.3.6** | Metadata pricing language matches paywall? |
| 7 | **2.3.11–2.3.13** | Cross-locale metadata materially consistent? |
| 8 | **3.1.1** | Digital goods unlocked via external purchase links? |
| 9 | **3.1.2** | Trial/auto-renew/cancel disclosures are legible sentences? |
| 10 | **4.2.1–4.2.2** | More than a thin WebView shell / template? |
| 11 | **4.8** | Third-party login → Sign in with Apple or valid exempt case? |
| 12 | **5.1.1(i)** | Privacy policy text matches code + PrivacyInfo? |
| 13 | **5.1.1(ii)** | Purpose strings specific and feature-tied? |
| 14 | **5.1.1(iii)** | Permissions/SDKs proportionate to app purpose? |
| 15 | **5.1.1(iv)** | Permission denial handled without forced loops? |
| 16 | **5.1.2** | ATT, tracking description, policy, and ad SDKs align? |
| 17 | **5.1.3** | HealthKit data not used for ads/marketing? |
| 18 | **5.1.4** | Kids signals → parental gate before links/IAP/account? |
| 19 | **5.4** | VPN → on-screen disclosure copy in UI strings? |
| 20 | **5.2.1–5.2.3** | Obvious trademark/brand misuse in metadata or UI? |
| 21 | **5.3.1–5.3.3** | Contest/sweepstakes copy includes official rules? |
| 22 | **5.6.2–5.6.3** | Developer identity consistent (support URL, domains, app name)? |

Use this prompt after Phase 3:

> You are **Pierre**. Phase 3 is done. Now run **Phase 4 deep review**: all 22 checks in
...
> Write explanations in `<USER_LANGUAGE>`. Do not change the scan verdict counts.
...
4. Present **Phase 4 deep review** — summary count (`REVIEW-FINDING` vs `REVIEW-PASS` of 22), then
   every `REVIEW-FINDING` with Pierre explanation; list `REVIEW-PASS` lines compactly or omit if all 22 passed.
